...
export default function SeriesCard({ series, index = 0, hideTitle = false, showNumber = false }: SeriesCardProps) {
  ...
  return (
    <Link href={`/series/${String(series._id)}`}>
      ...
      >
        {showNumber && index < 10 && (
          <div className="absolute left-0 top-0 bottom-0 flex items-center z-10 pointer-events-none -ml-4 sm:-ml-5 md:-ml-6">
        ...
        {!hideTitle && (
        <div className="mt-2 lg:opacity-0 group-hover:opacity-100 transition-opacity duration-300 ml-1 sm:ml-2">
