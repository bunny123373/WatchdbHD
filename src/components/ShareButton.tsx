    ...
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 p-3 bg-[#1F232D] rounded-lg shadow-xl z-50 min-w-[200px] border border-[#2a2f3d]">
          <p className="text-white text-sm font-medium mb-3">Share via</p>
          <div className="flex gap-2">
            {shareLinks.map((link) => (
              <button
                key={link.name}
                onClick={() => handleShare(link.url)}
                className={`${link.color} hover:opacity-80 w-10 h-10 rounded-lg flex items-center justify-center transition-opacity`}
                title={link.name}
              >
                <link.icon />
              </button>
