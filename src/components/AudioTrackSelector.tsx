...
interface AudioTrackSelectorProps {
  ...
  onTrackChange: (trackId: number | string) => void;
...
export default function AudioTrackSelector({
  ...
  const activeTrack = tracks.find((t) => String(t.id) === String(activeTrackId));
        ...
        {tracks.map((track) => (
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-sm transition-colors flex items-center gap-1.5 ${
              String(activeTrackId) === String(track.id)
                ? "bg-[#e50914] text-white"
                : "bg-white/10 text-white/70 hover:bg-white/20"
            }`}
          >
            <span>{getLanguageFlag(track.lang || track.language)}</span>
            <span>{track.name || track.label}</span>
            {String(activeTrackId) === String(track.id) && <Check className="w-3.5 h-3.5" />}
          </button>
        ))}
      </div>
    );
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span>Audio: {activeTrack?.name || activeTrack?.label || "Select"}</span>
          ...
          {tracks.map((track) => (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-base">{getLanguageFlag(track.lang || track.language)}</span>
                <span>{track.name || track.label}</span>
                {(track.lang || track.language) && (
                  <span className="text-xs text-white/40 uppercase">({track.lang || track.language})</span>
                )}
              </div>
              {String(activeTrackId) === String(track.id) && <Check className="w-4 h-4" />}
            </button>
          ))}
        </div>
      )}
