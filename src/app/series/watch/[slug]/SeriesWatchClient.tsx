...
export default function SeriesWatchClient({
  ...
  const getWatchedCountForSeason = (seasonNum: number) => {
    const season = series.seasons?.find((item) => item.seasonNumber === seasonNum);
    if (!season) return 0;
    return season.episodes.filter((episode) => watchedEpisodes.has(`${seasonNum}-${episode.episodeNumber}`)).length;
  };
  ...
  const playSeason = () => {
    const firstUnwatched = currentSeasonData?.episodes.find(
      (episode) => !watchedEpisodes.has(`${currentSeason}-${episode.episodeNumber}`)
    );
